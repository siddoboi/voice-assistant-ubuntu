...
On Pi Day 1 this is run once to populate every Pi-specific latency/RTF field
...
recordings/pi_benchmarks.json. The same script runs unchanged on WSL2 for a
dev baseline.
...
-----
    ...
    python scripts/benchmark_pi.py --input recordings/sample2.wav
    python scripts/benchmark_pi.py --output recordings/pi_benchmarks.json
"""
...
DEFAULT_OUTPUT = Path("recordings") / "pi_benchmarks.json"
...
def print_table(report: dict) -> None:
    ...
    print("\n" + "=" * 64)
    print(f"  Pi Benchmark Suite — input: {report['input']}")
    print("=" * 64)
